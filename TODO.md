...
### Step 1 : Data Loading
- Load the dataset from the `dataset_simpleclass.arff` file using `loadarff` method from `arff` library.
...
- Use the `resample` function to upsample the minority class. Set `replace=True` to allow for oversampling, `n_samples` to the length of the majority class and `random_state` using `random_state` variable.
...
- **Isotonic cv** :
   ...
   - Fulfill the metrics fonction with the test set
- **IVAP** :
   - Use `VennAbersCalibrator` to calibrate the model with **inductive** approach and **5 splits**, then fit the calibrated model on the training set
