...
data = arff.loadarff("../../data/dataset_stage_3.arff")
...
for i in range(y.shape[1]):
    ...
    pred_probs_calibrated.append(clf.predict_proba(X_test))
...
pred_probs_calibrated_matrix = np.vstack([p[:, 1] for p in pred_probs_calibrated]).T

pred_y_uncalibrated_matrix = np.array(pred_y_uncalibrated)
pred_y_calibrated_matrix = np.array(pred_y_calibrated).T
